...
Din AMAbot har nu samme lagdelte struktur som `/students` og `/teachers`: routes og data hver for sig, monteret via `express.Router()`. Statuskoder og fejlhåndtering for ugyldige id'er/kategorier — som stadig mangler, både her og i studerende-øvelsen — retter [øvelse 9](express-rest-api-amabot-sikkerhed-og-fejlhaandtering.md) op på, som en del af [RACE 7](../undervisning/024-race-7-sikkerhed-og-error-handling-25-09-2026.md), efter [REST API-øvelse: Fejlhåndtering](express-rest-api-fejlhaandtering.md) har bygget mønsteret på `/students`/`/teachers`. `client/`-mappen venter stadig på sit indhold — det retter [øvelse 8](fetch-dom-amabot.md) op på.
